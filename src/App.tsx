...
import boardTextureUrl from "./assets/board-texture.jpg";
import leopardBgUrl from "./assets/leopard-bg.jpg";
import inkomoImgUrl from "./assets/pieces/inkomo.png";
import ucuImgUrl from "./assets/pieces/ucu.png";
import leopardBallImgUrl from "./assets/pieces/leopard.png";
import isihlanguImgUrl from "./assets/pieces/isihlangu.png";
import {
...
// Real photo assets for each style. "isihlangu" is a shield+spears composition
// (not a plain sphere), so it's rendered without a circular clip, slightly larger,
// letting the spears extend past the point marker for visual flair.
const PIECE_IMAGE_URLS = {
  isihlangu: isihlanguImgUrl,
  inkomo: inkomoImgUrl,
  ucu: ucuImgUrl,
  leopard: leopardBallImgUrl,
};
const NON_CIRCULAR_STYLES = new Set(["isihlangu"]);

function PieceImage({ style, cx, cy, r }) {
  const url = PIECE_IMAGE_URLS[style];
  if (!url) return null;
  const clipId = `piececlip-${style}-${cx}-${cy}`;
  if (NON_CIRCULAR_STYLES.has(style)) {
    // preserve the shield+spears aspect ratio (roughly 0.85 wide : 1 tall), no clipping
    const w = r * 2.7;
    const h = w * (377 / 320);
    return (
      <image
  ...
  return (
    <svg viewBox="0 0 600 600" width="100%" height="100%" style={{ maxWidth: size, maxHeight: size }} role="img" aria-label="Mlabalaba board">
      ...
      {POINTS.map((pt, i) => {
        ...
        const effectiveStyle = isMine ? pieceStyle : opponentPieceStyle;
        const hasRealImage = occ && effectiveStyle && PIECE_IMAGE_URLS[effectiveStyle];
        return (
          <g key={i}>
            <circle
            ...
            {occ && hasRealImage && (
              <g className={isLastMoved ? "mlb-pop" : ""} pointerEvents="none">
                {!NON_CIRCULAR_STYLES.has(effectiveStyle) && (
                  <circle cx={pt.x} cy={pt.y} r={pr + 1} fill="#00000055" transform="translate(0,2)" />
                )}
                <PieceImage style={effectiveStyle} cx={pt.x} cy={pt.y} r={pr} />
                {!NON_CIRCULAR_STYLES.has(effectiveStyle) && (
                  <circle cx={pt.x} cy={pt.y} r={pr} fill="none" stroke={isSelected ? palette.gold : "#00000033"} strokeWidth={isSelected ? 4 : 1.5} />
                )}
                {isSelectable && (
...
function OnlinePanel({ palette, onBack, onEnterRoom, playerName, setPlayerName }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const code = await createRoom(createInitialState(), name);
      onEnterRoom(code, "P1", pieceStyle, name, null);
    } catch (e) {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      const { hostName } = await joinRoom(joinCode.trim().toUpperCase(), name);
      onEnterRoom(joinCode.trim().toUpperCase(), "P2", pieceStyle, name, hostName);
    } catch (e) {
    ...
    <div className="mlb-fade-in max-w-2xl mx-auto w-full flex flex-col gap-4">
      ...
      <div className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: "var(--mlb-surface)", border: "1px solid var(--mlb-border)" }}>
        ...
        <div className="flex flex-col gap-2">
          <p className="text-xs font-bold uppercase tracking-wide" style={{ color: "var(--mlb-textDim)" }}>Your name</p>
          <input
            value={playerName}
            ...
            maxLength={15}
            className="mlb-focus rounded-lg px-3 py-2 text-sm"
            style={{ background: "var(--mlb-surface2)", border: "1px solid var(--mlb-border)", color: "var(--mlb-text)" }}
          />
        </div>
        <div className="flex flex-col gap-2">
    ...
    <div className="mlb-fade-in max-w-5xl mx-auto w-full flex flex-col gap-8">
      ...
      <div className="rounded-2xl p-4 flex flex-wrap items-center justify-center gap-6" style={{ background: "var(--mlb-surface)", border: "1px solid var(--mlb-border)" }}>
        <div className="flex items-center gap-2">
          <Avatar name={playerName || "You"} tone={palette.copper} size={36} />
          <input
            value={playerName}
            onChange={(e) => setPlayerName(e.target.value.slice(0, 15))}
            placeholder="Your name"
            aria-label="Your name"
            maxLength={15}
            className="mlb-focus text-sm font-semibold bg-transparent border-b w-28"
            style={{ color: "var(--mlb-text)", borderColor: "var(--mlb-border)" }}
          />
        </div>
      ...
      <div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl p-5 flex flex-col gap-3" style={{ background: "var(--mlb-surface)", border: "1px solid var(--mlb-border)" }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${palette.teal}22`, color: palette.teal }}><Users size={20} /></div>
            <p className="font-bold" style={{ color: "var(--mlb-text)" }}>Local 2-player</p>
            <p className="text-xs" style={{ color: "var(--mlb-textDim)" }}>Pass and play on one device with a friend, full rules enforced.</p>
            <div className="grid grid-cols-2 gap-2">
              <input value={localP1} onChange={(e) => setLocalP1(e.target.value.slice(0, 15))} placeholder="Player 1 name" aria-label="Player 1 name" maxLength={15}
...
function GameView({ mode, difficulty, palette, onExit, stats, setStats, soundOn, roomCode, onlineRole, pieceStyle = "classic", customNames = null, onlineMyName = "", onlineOpponentName = null, setOnlineOpponentName }) {
  ...
  const names = mode === "ai" ? { P1: (customNames && customNames.P1) || "You", P2: `AI (${difficulty})` }
    : isOnline ? {
        P1: myRole === "P1" ? (onlineMyName || "You") : (onlineOpponentName || "Opponent"),
        P2: myRole === "P2" ? (onlineMyName || "You") : (onlineOpponentName || "Opponent"),
      }
    : { P1: (customNames && customNames.P1) || "Player 1", P2: (customNames && customNames.P2) || "Player 2" };
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToRoom(roomCode, (row) => {
      ...
      if (setOnlineOpponentName) {
        const theirName = myRole === "P1" ? row.guest_name : row.host_name;
        if (theirName) setOnlineOpponentName(theirName);
      }
    });
...
export default function App() {
  ...
  const [customNames, setCustomNames] = useState(null); // {P1, P2} for AI/local modes
  const [onlineOpponentName, setOnlineOpponentName] = useState(null);
  ...
  const enterOnlineRoom = (code, role, ps, myName, opponentName) => {
    gameKey.current += 1;
    setRoomCode(code);
    setOnlineRole(role);
    setMode("online");
    ...
    setCustomNames(null);
    setPlayerName(myName || playerName);
    setOnlineOpponentName(opponentName || null);
    setView("game");
  };

  return (
    <div
      className="mlb-root min-h-screen w-full flex flex-col"
      style={{
        ...cssVars,
        minHeight: "100vh",
